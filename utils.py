...
# Bcrypt의 최대 비밀번호 입력 길이 (바이트 기준). 이제 bcrypt를 사용하지 않지만, 변수는 남겨둡니다.
BCRYPT_MAX_LENGTH = 72

# CryptContext 초기화
# 환경 충돌이 잦은 'bcrypt' 대신, 기본 라이브러리로 처리 가능한 
# 'pbkdf2_sha256'을 사용합니다. 이는 추가 백엔드 설치 문제를 우회합니다.
pwd_context = CryptContext(schemes=["pbkdf2_sha256", "sha256_crypt"], default="pbkdf2_sha256", deprecated="auto")

def truncate_password(password: str) -> str:
    """
    비밀번호를 최대 72바이트로 안전하게 자르는 헬퍼 함수입니다.
    """
    # pbkdf2_sha256은 길이 제한이 훨씬 덜 엄격하므로 이 로직은 안전 조치로만 작동합니다.
...
def hash_password(password: str) -> str:
    ...
    비밀번호를 해시하여 저장합니다.
    """
    # pbkdf2_sha256은 길이 제한이 덜 엄격하므로, truncate 호출 없이 password를 그대로 전달합니다.
...
def verify_password(plain_password: str, hashed_password: str) -> bool:
    ...
    """
    try:
        # pbkdf2_sha256은 길이 제한이 덜 엄격하므로, password를 그대로 전달합니다.
        # 기존에 저장된 sha256_crypt 해시도 이 함수에서 자동으로 검증됩니다.
